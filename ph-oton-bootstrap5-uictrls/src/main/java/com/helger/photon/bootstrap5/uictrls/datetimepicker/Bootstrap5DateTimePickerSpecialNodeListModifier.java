...
public final class Bootstrap5DateTimePickerSpecialNodeListModifier implements IHCSpecialNodeListModifier
{
  ...
  public ICommonsList <? extends IHCNode> modifySpecialNodes (@NonNull final ICommonsList <? extends IHCNode> aNodes)
  {
    ...
    final ICommonsList <Bootstrap5DateTimePickerJS> aDTPs = new CommonsArrayList <> ();
    int nFirstIndex = -1;
    int nIndex = 0;
    for (final IHCNode aNode : aNodes)
    {
      if (aNode instanceof final Bootstrap5DateTimePickerJS aJS)
      {
        aDTPs.add (aJS);
        if (nFirstIndex < 0)
          nFirstIndex = nIndex;
    ...
    final ICommonsList <Bootstrap5DateTimePickerJS> aRest = aDTPs.getClone ();
    while (aRest.isNotEmpty ())
    {
      final Bootstrap5DateTimePickerJS aCurrent = aRest.removeFirstOrNull ();
      ...
      final ICommonsList <Bootstrap5DateTimePickerJS> aSameOptions = new CommonsArrayList <> ();
      final Iterator <Bootstrap5DateTimePickerJS> itRest = aRest.iterator ();
      while (itRest.hasNext ())
      {
        final Bootstrap5DateTimePickerJS aCurrentRest = itRest.next ();
